...
description: 明源场宣图片批量规范化处理：生成/审计素材、2K 横竖规范化、轻度优化、PNG 素材叠加、HTML多版本预览、过程预览与打包交付。
---
...
> 版本: 0.3.3
> 作者: Alix（明源云海南）
> 用途: 批量场宣图片规范化处理——格式标准化 + 轻度优化 + 素材叠加 + 过程预览 + HTML多版本预览 + 打包交付
...
🔴 铁律3：背景选图必须先 vision_analyze 确认 logo 可见性（见门禁C）
...
搜索/下载背景图后，**必须先 `vision_analyze` 逐张检查**是否适合 logo 叠加：

- ❌ 不适合：画面过于复杂（立交桥/密集星空/纹理太多）、缺乏留白区域、主体在画面中心
- ✅ 适合：有干净天空区域（上1/3）、大面积纯色水面、简洁渐变、主体在边缘

**禁止**：不检查直接使用背景图。logo 被复杂背景淹没是常见失误，必须审计后再进入叠加。

**正确做法：**
1. 从 picsum 下载 10+ 张不同 seed 的图片
2. 用 `vision_analyze(image_url=path, question="...")` 逐张评估
3. 只选用 vision 确认"适合放logo"的图片
4. 再进入阶段1规范化流程
...
- 用户发送 zip 压缩包（**仅素材无原图** → 走方案C/D：联网搜壁纸作背景）
...
**本 skill 触发的任务走主agent直接处理**，所有步骤在主线程完成（无需经过 Kanban dispatch 或 izuer-creative 协助）。

执行流程：
1. **主agent加载本 skill 直接执行** — 解包→审计→搜索壁纸→规范化→优化→叠加→预览→交付
2. **不调用 `delegate_task`**（每步需顺序依赖，不适合并行）

---

## ⚠️ 双logo缩放陷阱（2026-05-25 真实修复）

**问题**：`place_dual_logos_top_right` 如果直接用两个logo的原始宽高计算组宽后整体缩放，
当两个logo宽高比差异大时（如明源logo 1504×405≈3.7:1 vs 研究院 1090×200≈5.45:1），
会产生严重比例失衡——宽高比大的那个会被放大到异常尺寸。

**修复后的正确做法（两步法）：**
1. **先统一高度**：以 target_h=60px 为目标，分别缩放两个logo（等高等宽比）
2. **再组缩放**：将组合体一同缩放到不超过背景宽35%

**验证**：实测研究院从 1090×200 → 299×55，明源logo 1504×405 → 241×65，
总宽556px (21.7%)，视觉平衡。详见 `references/common-logo-patterns-528.md` 踩坑记录。
...
[阶段0] 解包 & 素材审计
    ...
    │  未说明则询问每张素材的放置位置
    │  ┌─ 有原图 → 输出: 素材审计图 + 原片审计图
    │  └─ 仅素材 → 输出: 素材审计图 + 询问用户背景方案
    │              ├─ 联网搜壁纸 → 搜索→下载（见references/common-logo-patterns-528.md 方案C）
    │              └─ 纯色/渐变 → Python Pillow生成背景
    │              输出: 壁纸/背景进入阶段1
    │
    ▼
[阶段1] 格式规范化 ── 2K标准
...
| 描述 | 计算方式 |
|------|----------|
| 右上角（单logo） | 距右边 40px, 距上边 40px |
| 右下角（单logo） | 距右边 40px, 距下边 40px |
| 左上角（单logo） | 距左边 40px, 距上边 40px |
| 左下角（单logo） | 距左边 40px, 距下边 40px |
| 右上角双logo | 研究院（左）+ 明源logo（右）并排，间距16px，整体距右边40px/上边40px。先统一文字大小感，再整体缩放至背景宽35%以内（详见references/common-logo-patterns-528.md 方案A） |
| 底部全幅 | logo等比缩放至长边=2560px，y=bg_h-asset_h，左右填满（详见references/common-logo-patterns-528.md 方案A） |

### 阶段4：打包交付

1. 最终产物放入 `成果/` 目录
2. **生成 HTML 多版本预览页**：调用 `工具/generate_html_preview.py`，将所有成品图片按背景分组、变体分类，生成可交互的 HTML 预览（浏览器直接打开，点击大图放大，支持键盘 Esc 关闭）
   - 适用场景：有多个变体（不同背景/不同logo大小）时
   - 输出位置：`成果/preview.html`
   - **必须主动发给用户**（门禁B），用户可直观对比所有变体
3. 打包 zip：`成果/场宣图_成品_YYYYMMDD_taskname.zip`
4. 发送用户，留任务日志
...
| `工具/mingyuan528_test_pipeline.py` | 528场宣测试样张全链路处理 | 素材zip | 成品PNG+JPG |
| `工具/generate_html_preview.py` | 生成HTML多版本预览页 | 成品目录 | preview.html |
...
- `references/common-logo-patterns-528.md` — 明源528活动常见logo排布方案（双logo右上角/底部全幅/无原图处理）
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v0.3.3 | 2026-05-25 | 新增 generate_html_preview.py 成品HTML多版本预览页，集成到阶段4打包交付流程并纳入门禁B推送 |
| v0.3.2 | 2026-05-25 | 路由降级（主agent直接处理）、门禁C背景可见性审计、dual-logo缩放两步法修复、mingyuan528_test_pipeline.py 测试脚本 |
...
> 版本: 0.3.3 | 作者: Alix（明源云海南） | 更新时间: 2026-05-25
